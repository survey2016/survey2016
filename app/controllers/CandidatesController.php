...
class CandidatesController extends BaseController {
	...
	*/
	public function main(){
		$results = DB::select("SELECT * FROM candidates");
		$data = array(
			'title' => 'Candidates',
			'results'=>$results
		);
		return View::make('candidates',$data);
	}
	...
	public function getCandidate($id){	
		...
		//CANDIDATES
		$candidateStatus = array(); $pieChartData = array(); $voteName = array(); $nameVote = array();
		$candidateIDs = array(); $positionIDs = array();
		$listOfCandidates = DB::select("SELECT * FROM candidates ORDER BY positionID ASC");
		foreach ($listOfCandidates as $c) {
        	...
        	$voteName[$c->positionID][$surveyData] = $name;
        	$nameVote[$c->positionID][$name] = $surveyData;
        	$candidateIDs[] = $c->id;
        	$positionIDs[] = $c->positionID;
		}

		// print_r($pieChartData[1]); exit();
		$data = array(
			'title' => 'Candidate',
		...
		return View::make('candidate-info',$data);	
	}
